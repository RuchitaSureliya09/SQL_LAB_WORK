...
SELECT REPLICATE('LENCY ',3) AS REPLICATE_FUN
...
SELECT SUBSTRING(FIRSTNAME,2,LEN(FIRSTNAME)-1) AS SUBSTRING_FUN FROM EMPLOYEE

--25. Display FIRSTNAME after replacing vowels with '*'.
--SELECT REPLACE(FIRSTNAME,[AEIOU],'*') AS REPLACE_VOWEL_STAR FROM EMPLOYEE
...
SELECT REVERSE(FIRSTNAME) AS REVERSE_FIRSTNAME FROM EMPLOYEE
