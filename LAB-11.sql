...
SELECT YEAR(ADATE) AS YEARS,COUNT(*) AS ACT_NO 
FROM DEPOSIT
GROUP BY YEAR(ADATE)

--22. Display number of accounts opened in each month.
SELECT MONTH(ADATE) AS MONTHS,COUNT(*) AS ACT_NO 
FROM DEPOSIT
GROUP BY MONTH(ADATE)

--23. Display maximum amount deposited in each year.
SELECT YEAR(ADATE) AS YEARS,MAX(AMOUNT) AS MAX_AMOUNT 
FROM DEPOSIT
GROUP BY YEAR(ADATE)

--24. Display minimum amount deposited in each month.
SELECT MONTH(ADATE) AS MONTHS,MIN(AMOUNT) AS MIN_AMOUNT 
FROM DEPOSIT
GROUP BY MONTH(ADATE)

--25. Display total amount deposited in each year.
SELECT YEAR(ADATE) AS YEARS,SUM(AMOUNT) AS TOTAL_AMOUNT 
FROM DEPOSIT
GROUP BY YEAR(ADATE)
...
SELECT DATEDIFF(DAY,ADATE,GETDATE()) AS DIFFERENCES 
FROM DEPOSIT
